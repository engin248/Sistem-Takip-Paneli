...
export default function TaskCard({ task }: { task: Task }) {
  ...
  const handleArchive = async () => {
    if (!confirm('GÖREV ARŞİVLENSİN MI?')) return;
    setIsArchiving(true);
    try {
  ...
  return (
    <div className={`flex items-center justify-between p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm hover:shadow-md transition-all ${isAnyLoading ? 'opacity-60 pointer-events-none' : ''}`}>
