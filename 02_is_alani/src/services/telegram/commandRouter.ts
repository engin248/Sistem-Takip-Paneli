...
export function registerHandlers(botInstance: Bot): void {
  ...
  botInstance.command('yardim', async (ctx) => {
    await sendReply(ctx, [
      `📖 <b>KOMUT LİSTESİ — @Sistem_takip_bot</b>`,
      ``,
      `<b>📋 GÖREV YÖNETİMİ:</b>`,
      `💬 <b>Metin mesaj</b> → Görev olarak kaydedilir`,
      `🎤 <b>Sesli mesaj</b> → AI ile çevirilip görev olarak kaydedilir`,
      `/gorevler — Aktif görev listesi (son 15)`,
      `/gorev TSK-xxx — Görev detayı`,
      `/tamamla TSK-xxx — Görevi tamamla`,
      `/iptal TSK-xxx — Görevi iptal et`,
      ``,
      `<b>🔍 VERİ SORGU (RAG):</b>`,
      `/ara <kelime> — Görev + log'larda canlı arama`,
      `/rapor — Sistem özet raporu`,
      `/ajanlst — Ajan kadrosu durumu`,
      ``,
      `<b>🌐 İNTERNET:</b>`,
      `/web <sorgu> — DuckDuckGo ile internet araması`,
      ``,
      `<b>🔧 SİSTEM ARAÇLARI:</b>`,
      `/calistir bootstrap — Sistem sağlık kontrolü`,
      `/calistir webhook — Webhook durumu`,
      `/calistir temizle — Eski görevleri arşivle`,
      ``,
      `<b>ℹ️ DİĞER:</b>`,
      `/start — Başlangıç`,
      `/durum — Sistem durumu`,
