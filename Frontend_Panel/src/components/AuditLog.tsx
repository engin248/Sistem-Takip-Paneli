  ...
  return (
    <section>
      <div className={`flex justify-between items-center mb-4 ${dir === 'rtl' ? 'flex-row-reverse' : ''}`}>
        <h2 className="text-sm font-bold text-slate-300 drop-shadow-[0_0_5px_currentColor] tracking-widest uppercase text-start">
          {tr.auditTitle}
          <span className="ms-2 text-[10px] font-black bg-slate-700/80 text-white px-2 py-0.5 rounded-full drop-shadow-sm border border-slate-500/50">
            {logs.length}
          </span>
        </h2>
        <button onClick={loadLogs} className="text-[10px] text-blue-400 hover:text-blue-300 hover:underline font-bold transition-colors">{tr.refresh}</button>
      </div>
      <div ref={scrollRef} className="space-y-2 max-h-80 overflow-y-auto scrollbar-thin scroll-smooth pr-1">
        {logs.length === 0 && <p className="text-[10px] text-slate-400 italic">{tr.noRecords}</p>}
        {logs.map((log) => (
          <div key={log.id} className="p-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/50 rounded-lg text-start hover:bg-slate-800 transition-colors">
            <div className={`flex justify-between mb-1 ${dir === 'rtl' ? 'flex-row-reverse' : ''}`}>
              <span className="text-[10px] font-bold text-blue-400 font-mono drop-shadow-[0_0_5px_currentColor]">
            ...
            <p className="text-[11px] font-medium text-slate-600 dark:text-slate-200 leading-relaxed">
