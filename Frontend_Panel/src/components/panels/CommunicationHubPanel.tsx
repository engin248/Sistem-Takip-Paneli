...
export default function CommunicationHubPanel() {
    ...
    return (
        <div className="flex flex-col h-full w-full bg-[#030712] animate-fade-in overflow-hidden">
            ...
            <div className="shrink-0 p-4 md:p-6 border-b border-white/5 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-[#0b1120]/50 backdrop-blur-md">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            <span className="w-1.5 h-1.5 bg-emerald-500 rounded-none shadow-[0_0_10px_#10b981]"></span>
                            <p className="text-[10px] font-mono text-emerald-400 uppercase tracking-widest leading-none small-caps">HUB AKTİF • 4 KANAL BAĞLI</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                    {(['ALL', 'TELEGRAM', 'WHATSAPP', 'DISCORD', 'SMS'] as const).map(p => (
                        <button
                            ...
                            className={`px-4 py-2 text-[9px] font-black tracking-widest border transition-all ${selectedChannel === p ? 'bg-cyan-500 text-white border-cyan-400' : 'bg-slate-900 border-slate-800 text-slate-500 hover:border-slate-600'
                                }`}
                        >
                            {p}
                        </button>
            ...
            <div className="flex-1 flex flex-col lg:flex-row min-h-0">
                ...
                <div className="flex-1 flex flex-col border-r border-white/5 bg-[#030712]">
                            ...
                            <div key={msg.id} className={`flex w-full ${msg.type === 'OUT' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[90%] md:max-w-[70%] group relative ${msg.type === 'OUT' ? 'items-end' : 'items-start'} flex flex-col gap-2`}>
                                    ...
                                    <div className={`p-4 border shadow-2xl relative ${msg.type === 'OUT'
                                        ...
                                        <div className={`absolute top-0 bottom-0 w-1 ${msg.type === 'OUT' ? 'right-0 bg-cyan-500' : 'left-0 bg-slate-700'}`}></div>
                                        ...
                                        <div className="absolute -bottom-2 -right-2 opacity-20 group-hover:opacity-60 transition-opacity">
                                            {msg.platform === 'WHATSAPP' && <Phone className="w-5 h-5 text-emerald-400" />}
                                            {msg.platform === 'TELEGRAM' && <Send className="w-5 h-5 text-blue-400" />}
                                    ...
                                    <div className={`flex items-center gap-2 mt-1 px-1 ${msg.type === 'OUT' ? 'flex-row-reverse' : ''}`}>
                                        ...
                                        <CheckCircle className={`w-2.5 h-2.5 ${msg.status === 'READ' ? 'text-cyan-400' : 'text-slate-700'}`} />
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    {/* MESSAGE INPUT HUB */}
                    <div className="shrink-0 p-4 bg-[#0b1120]/80 border-t border-white/5 backdrop-blur-xl">
                        <div className="flex items-center gap-3 bg-slate-950 border border-slate-800 p-2 group focus-within:border-cyan-500/50 transition-all">
                            ...
                            <button
                                ...
                                className="px-8 py-3 bg-cyan-500 text-white font-black text-[10px] uppercase tracking-[0.2em] shadow-[0_0_20px_rgba(6,182,212,0.3)] hover:bg-cyan-600 disabled:opacity-50 transition-all flex items-center gap-3"
                            >
                ...
                <div className="w-full lg:w-96 bg-[#060912] p-6 space-y-8 overflow-y-auto custom-scrollbar border-t lg:border-t-0 lg:border-l border-white/5">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 gap-2">
                            {[
                                ...
                                <button key={i} className="w-full p-4 bg-slate-900/50 border border-slate-800 hover:border-amber-500/30 hover:bg-amber-500/5 transition-all text-start group">
                                    <div className="flex items-center gap-3">
                                        <div className="p-2 bg-slate-950 text-slate-500 group-hover:text-amber-400 transition-colors">{b.icon}</div>
                    ...
                    <div className="space-y-4">
                        <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] flex items-center gap-2">
                            <Layers className="w-4 h-4 text-cyan-500" /> KANAL DURUMLARI
                        </h3>
                        <div className="space-y-3">
                            {[
                                { platform: 'TELEGRAM BOT', status: 'BAĞLI', color: 'text-emerald-400' },
                                { platform: 'WHATSAPP BIZ', status: 'BAĞLI', color: 'text-emerald-400' },
